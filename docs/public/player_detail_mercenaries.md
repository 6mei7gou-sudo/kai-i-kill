...
依頼は**レーティング制**で管理される。登録した討伐者にはランクがつき、高ランクの者に高難度の依頼が回る。

| ランク帯 | 主な依頼 |
|----------|----------|
| 低ランク | 五級怪異の駆除案件が中心。量産型の日常業務 |
| 中ランク | 四級の複数同時発生案件・三級の補助要員としての参加 |
| 高ランク | 三級以上の高額案件・指名依頼 |

実績と評判に応じて**指名依頼**が入る。指名は報酬が高いが、失敗すれば信用とランクを失う。ネットワーク内での情報交換・装備の融通・チーム編成の斡旋も行われる。
