...
1. **スキル効果はテキストパースで判定**: skillData.jsのeffect文字列をregexで解析してゲーム処理に変換している。新しいスキルを追加する場合、effectの文言がgameEngineのパターンにマッチするよう注意が必要。（対応済み: v3で攻撃系スキル判定を総合キーワード方式に拡張）

### 解消済みの問題（v3.1）

- ~~Coop戦闘にスキル未実装~~ → 各キャラに `loadBattleSkills` + `applyPassiveSkills` を適用
- ~~配属スキルが空で読み込まれない~~ → 所属からデフォルト配属を推定するフォールバック追加（祓部→機動班、傭兵→突撃型、無所属→野良討伐者）
- ~~負傷ペナルティが見た目だけ~~ → HP50%以下で全判定に-1を実際に適用（通常攻撃・魔法・スキル）
- ~~通常攻撃の判定属性が識固定~~ → 武器型に応じた属性を使用（`weaponDetails.ability` → `getAttackAbility` フォールバック）
- ~~敵の防御/守護が効果なし~~ → 防御態勢=被ダメ-2、核守護=核へのダメージ-2（守護中の護衛1体につき）
