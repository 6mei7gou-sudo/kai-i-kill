...
- 地理の表記揺れ・未確定事項の決定（`docs/gm/geography.md` 9章）と、決定後の関連ファイル同期
- `docs/player/geography.md` をサイトページに紐付けるか（siteDocs.js・`/world/` 章構成）の判断

## 変更ログ（新しい順）

- 2026-09-08：土地・地理情報を統合。`docs/gm/geography.md` を v1.1 に全面改稿（世界観バイブル・勢力資料・年表・用語集・デザインガイド・公式地図v1.1の凡例を反映、侵食率表記を除去）し、秘匿除去版 `docs/player/geography.md` を新設（ページ未紐付け）。資料間の不一致（灰嶺市の読み「かれいし／かいれいし」、底澱「そこよ／そこよど」、白銀の廃線の路線種別、雷禽本社所在地、白鷺調査社の拠点、widowkiller本店、御神楽事変の現場、Webゲーム依頼主「関東管区」）は gm/geography.md 9章「表記揺れ・未確定事項」に集約。同日に決定：灰嶺市＝かいれいし、底澱＝そこよ、白銀の廃線＝凍月嶺の山間鉄道、雷禽本社＝雷ノ湊（関連ファイルに反映済み）。widowkiller本店と御神楽の所在は設定しない。公式地図v1.1の「かれいし」表記は次回改版時に修正
