...
この架空日本は五つの地方管区に分けられている。東管区の灰嶺市（かいれいし）が最大都市圏であり怪異発生件数も全国最多。中管区の千隼市（ちはやし）に魔導省本庁舎と祓部中央本部がある。西管区には傭兵文化が根づいた旧港湾都市、北管区には禁足地が密集する山岳地帯、南管区には蜃ノ群島（しんのぐんとう）と呼ばれる孤立した島嶼部が広がる。
...
SNSは日常のコミュニケーション手段だ。最も普及しているのは**MirrorLine（ミラーライン）**だ。メッセージ・タイムライン・グループ機能を統合した総合プラットフォームであり一般市民から討伐者まで幅広く利用されている。MirrorLine上の都市伝説まとめチャンネル・怪談投稿グループ・心霊スポット共有コミュニティが人気コンテンツとして消費されている。利用者はフィクションとして楽しんでいるが、討伐者から見ればMirrorLineで拡散される噂が怪異の発生源になりうるという皮肉な構造がある。
...
怪異の存在の中心であり消滅条件の鍵だ。基本的には**物に宿る**。特定の場所・道具・建造物・記録媒体などだ。稀に死体や生きた人間に宿る場合があり、その場合は対処の難度と倫理的複雑さが跳ね上がる。
...
現代の情報社会で生まれた怪異だ。SNS・都市伝説・ネットロアが発生源。拡散速度が強さに比例する可能性がある。強さは低めだが**初見殺しと未知数**が最大のリスクだ。データベースに記録がなく対処法が存在しない。
...
社会に深く根を張った怪異だ。核を破壊しても噂が生き続ける限り新たな核を形成して再出現する。討伐は根絶ではなく**制圧**だ。完全根絶には噂そのものを消す必要があるがこれは現実的にほぼ不可能だ。
...
| 等級 | 位置づけ | 基準 |
|------|----------|------|
| 特級 | 災害 | 現状の手段では消滅が極めて困難。封印・管理が限界 |
| 一級 | 大ボス | 信念密度が非常に高い。長い歴史を持つ強力な怪異 |
| 二級 | ボス | 記録のある古い怪異の大半 |
| 三級 | 中ボス | 拡散が進んだ新しい怪異など。化け物じみた強さだが力押しも不可能ではない |
| 四級 | 雑魚 | 発生したばかりの新しい怪異 |
| 五級 | 雑魚 | 魔法事故由来の微小怪異。自然消滅することが多い |
...
| 種別 | 基準 |
|------|------|
| 甲種 | 無差別に人を害する。接触した全員が標的 |
| 乙種 | ルールを破った者など条件付きで害する |
| 丙種 | 刺激しなければ安全。放置が最善の場合がある |
...
| 等級 | 必要な段階 | 備考 |
|------|------------|------|
| 五級・四級（雑魚） | 戦闘のみ | 魔法や物理で直接倒せる。日常的に湧く害獣だ |
...
討伐者の間で共有されている無力化方法は**破壊**だ。怪異の核を物理的・霊的・魔法的手段で破壊することで怪異を消滅させる。
...
| 言語名 | 属性 | アプローチの質感 |
|--------|------|------------------|
| **P** | 基礎言語 | 全魔法の土台。魔導具製造の基幹。何にでも組み合わせられる汎用言語 |
| **Igniscript** | 赤魔法 | 燃やす・爆発させる・熱で変容させる。力任せで直線的 |
| **Lupis Surf** | 青魔法 | 流す・包む・圧力で押し動かす。柔軟で範囲が広い |
| **Ivyo** | 緑魔法 | 育てる・自然のサイクルに乗せる。即効性より持続性 |
| **NGT** | 黄色魔法 | 加速・電気的処理・情報として解析する。デジタルインフラと親和性が高い |
| **Monyx** | 無色魔法 | 最小限の術式で組む。どんな方向にも転用できる。魔導具調整の主力 |
| **P:** | 紫魔法 | 弱体化・妨害・封印。対象を削ぐ・縛る・封じる。Pの派生で習得者が少ない |
| **P'** | 桃魔法 | 回復・強化・修復。傷を癒す・能力を底上げする。Pの派生で身体的負荷が大きい |
...
| 分類 | 主な用途 | 怪異発生リスク | 扱いやすさ | PC選択 |
|------|----------|----------------|------------|--------|
| 武装型 | 近中距離戦闘・補助 | 低 | 高い | 可 |
| 独立型 | 偵察・支援・包囲 | 中 | 機体依存 | 可 |
| 半装身型 | 部位強化・中火力 | 中 | 中程度 | 可 |
| 全装身型 | 高火力・高防御 | 高 | 低い | **不可** |
| 搭乗型 | 機動・輸送・制圧 | 中 | 中程度 | 可 |
| 戦闘用搭乗型 | 大型怪異討伐 | 非常に高い | 非常に低い | **不可** |
...
| **主な依頼** | 五級怪異の討伐が大半。日常的に湧く雑魚の駆除が討伐者の基本業務だ |
...
灰嶺市最下層の底澱（そこよど）をはじめとするスラムには**リトルクラン**と呼ばれる子供たちの寄り合いが存在する。親を怪異に奪われた孤児や行き場のない子供たちが互いに身を寄せ合い、生き延びるために集まった小集団だ。正規の訓練を受けていないが、怪異と隣り合わせの生活の中で実戦的な勘を身につけている。
...
| ショック覚醒型 | 怪異に関わる強烈な体験が引き金。恨みが動機に直結しやすい |
| 実験覚醒型 | 何者かによる人体実験で強制覚醒。素養強度が高い代わりに代償が大きい |
| 接触覚醒型 | 怪異の核や特殊な素材への長期接触で覚醒。変質リスクが伴う |
...
| 第二極 | 独立系企業・傭兵集団 | 利益。国家にも危険な勢力にも必要とされる立場を維持して生き残る |
| 第三極 | 正体不明の組織 | 既存の権力構造の外側。怪異そのものを利用する勢力が存在するという情報がある |
...
現在、怪異の扱いに対する考え方は、大きく分けて2つの主義に分類される。

### 怪異排除主義

怪異を人間社会における脅威とみなし、破壊や封印によって無力化することを目的とした考え方だ。討伐者の共通思想であり、国の方針もこれに属する。

### 怪異利用主義

怪異を新たな資源や道具とみなし、管理や研究によって、かつての魔法のように人間社会に取り入れようという考え方だ。技術部や封印班の中には、こうした思想を持つ者が少なくない。また、傭兵の間で報酬のために不当に依頼を発生させるといった行為もこれに該当する。

---

## 16. 稀人（まれびと）とは

異世界出身の人間、もしくはそれに類似した知的生命体に対する呼称だ。出身世界の文明レベルは、原則として現世と同等、もしくはそれ以下に限られる。

およそ人間と呼べるシルエットだが、特異な外見的特徴（身体の一部萎縮や肥大化、他生物に類似した形質など）を持つことがある。

また、多くのケースで言語的コミュニケーションが成立する。他言語を用いる場合でも、現世の翻訳技術により円滑な意思疎通が取れるものとする。
...
3. **能力スタイル**：魔法（直接行使または魔導具運用）。異能はNPC専用であり、PCは選択できない。
