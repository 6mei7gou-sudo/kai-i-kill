...
// 等級に応じたHP目安
const HP_GUIDE = {
    '五級': '5〜10', '四級': '10〜20', '三級': '20〜35',
    '二級': '35〜60', '一級': '60〜100', '特級': '100以上',
};

// 等級の色
const GRADE_COLOR = {
    '特級': 'var(--accent-danger)', '一級': 'var(--accent-gold)',
    '二級': 'var(--accent-blue)', '三級': 'var(--text-primary)',
    '四級': 'var(--text-secondary)', '五級': 'var(--text-muted)',
    '不明': 'var(--text-muted)',
};

// 脅威度の色
const THREAT_COLOR = {
    '甲種': 'var(--accent-danger)', '乙種': 'var(--accent-gold)',
    '丙種': 'var(--text-secondary)', '丁種': 'var(--accent-blue)',
    '不明': 'var(--text-muted)',
};
...
export default function AnomalyForm() {
    ...
    const updateItem = (key, idx, val) => setForm(prev => {
        const arr = [...prev[key]];
        arr[idx] = val;
        return { ...prev, [key]: arr };
    });
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!form.anomaly_name.trim()) {
            setResult({ ok: false, msg: '怪異の通称は必須です' });
            return;
        }
        setSubmitting(true);
        setResult(null);
    ...
    // 動的リスト入力
    const DynamicList = ({ label, items, itemKey, placeholder, hint }) => (
        <div style={S.fieldGroup}>
            <label style={S.label}>{label}</label>
            {items.map((item, i) => (
                <div key={i} style={{ display: 'flex', gap: '8px', marginBottom: '6px' }}>
                    <input style={{ ...S.input, flex: 1 }} value={item}
                        onChange={e => updateItem(itemKey, i, e.target.value)}
                        placeholder={`${placeholder} ${i + 1}`} />
                    {items.length > 1 && (
                        <button type="button" style={{ ...S.addBtn, color: 'var(--accent-danger)', borderColor: 'rgba(255,77,77,0.3)' }}
                            onClick={() => removeItem(itemKey, i)}>✕</button>
                    )}
                </div>
            ))}
            <button type="button" style={S.addBtn} onClick={() => addItem(itemKey)}>+ 追加</button>
            {hint && <div style={S.hint}>{hint}</div>}
        </div>
    );
        ...
        <div className="container">
            ...
            <form onSubmit={handleSubmit}>
                ...
                <div style={S.section}>
                    ...
                    <div style={S.row}>
                        <Input label="投稿者名" value={form.author_name} placeholder="@ユーザー名"
                            onChange={v => set('author_name', v)} />
                        <Select label="公開範囲" value={form.visibility}
                            onChange={v => set('visibility', v)} options={['公開', '限定']} />
                        <Select label="調査状態" value={form.status}
                ...
                <div style={S.section}>
                    ...
                    <div style={S.row}>
                        <Select label="存在強度等級" value={form.grade}
                            onChange={v => set('grade', v)} options={OPTIONS.grades}
                            hint={HP_GUIDE[form.grade] ? `HP目安: ${HP_GUIDE[form.grade]}` : ''} />
                        <Select label="脅威度種別" value={form.threat_type}
                            onChange={v => set('threat_type', v)} options={OPTIONS.threats} />
                        <Select label="影響範囲" value={form.influence_range}
                            onChange={v => set('influence_range', v)} options={OPTIONS.ranges} />
                        <Select label="被害性質" value={form.damage_type}
                            onChange={v => set('damage_type', v)} options={OPTIONS.damages} />
                    </div>
                    ...
                    <div style={S.fieldGroup}>
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                            {OPTIONS.tagOptions.map(tag => (
                                <button key={tag} type="button" style={S.tagBtn(form.tags.includes(tag))}
                ...
                <div style={S.section}>
                    ...
                    <Input label="怪異の通称 *" value={form.anomaly_name}
                        onChange={v => set('anomaly_name', v)} placeholder="例：深夜廻り" />
                    <TextArea label="一行要約" value={form.summary}
                        onChange={v => set('summary', v)} placeholder="例：◯◯すると必ず□□が起きる噂" />
                    <TextArea label="典型パターン" value={form.typical_pattern}
                        onChange={v => set('typical_pattern', v)} placeholder="何が起きる？順番は？" />
                    <div style={S.row}>
                        <TextArea label="前兆" value={form.omen}
                            onChange={v => set('omen', v)} placeholder="遭遇前に起きる兆候" />
                        <TextArea label="最悪ケース" value={form.worst_case}
                            onChange={v => set('worst_case', v)} placeholder="推定でも可" />
                    </div>
                </div>
                ...
                <div style={S.section}>
                    ...
                    <TextArea label="元ネタ（噂の種）" value={form.origin}
                        onChange={v => set('origin', v)} placeholder="伝承 / 学校 / 掲示板 / 動画 / チェーンDM 等" />
                    <TextArea label="拡散経路" value={form.spread_route}
                        onChange={v => set('spread_route', v)} placeholder="SNS / 配信 / クローズドコミュニティ 等" />
                    <div style={S.fieldGroup}>
                        <label style={{ ...S.label, display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
                            <input type="checkbox" checked={form.distorted_countermeasure}
                                onChange={e => set('distorted_countermeasure', e.target.checked)} />
                            "歪んだ対策" が流通している
                        </label>
                    </div>
                    {form.distorted_countermeasure && (
                        <div style={S.row}>
                            <TextArea label="本来の対策（推定）" value={form.original_countermeasure}
                                onChange={v => set('original_countermeasure', v)} />
                            <TextArea label="現在流通している対策" value={form.current_countermeasure}
                                onChange={v => set('current_countermeasure', v)}
                                hint="危険なら赤字で注意喚起" />
                        </div>
                    )}
                </div>
                ...
                <div style={S.section}>
                    ...
                    <div style={S.row}>
                        <Select label="核の種別（推定）" value={form.core_type}
                            onChange={v => set('core_type', v)} options={OPTIONS.coreTypes} />
                        <Select label="核の挙動（推定）" value={form.core_behavior}
                            onChange={v => set('core_behavior', v)} options={OPTIONS.coreBehaviors} />
                        <Select label="核の破壊可否（推定）" value={form.core_destroyable}
                            onChange={v => set('core_destroyable', v)} options={OPTIONS.coreDestroyable} />
                    </div>
                    <DynamicList label="核の候補" items={form.core_candidates} itemKey="core_candidates"
                        placeholder="場所 / URL / 物品名 など" />
                </div>
                ...
                <div style={S.section}>
                    ...
                    <DynamicList label="発動条件（トリガー候補）" items={form.triggers} itemKey="triggers"
                        placeholder="トリガー" hint="怪異が動き出すきっかけ" />
                    <DynamicList label="禁忌（やってはいけない候補）" items={form.taboos} itemKey="taboos"
                        placeholder="禁忌" hint="破るほど逃げられなくなる" />
                    <TextArea label="例外・抜け道（候補）" value={form.loopholes}
                        onChange={v => set('loopholes', v)} placeholder="例：△△を先に行えばセーフ、らしい" />

                    <label style={{ ...S.label, marginTop: 'var(--space-lg)' }}>違反時の症状（段階）</label>
                    <div style={S.row}>
                        <Input label="初期" value={form.violation_early}
                            onChange={v => set('violation_early', v)} placeholder="初期症状" />
                        <Input label="中期" value={form.violation_mid}
                            onChange={v => set('violation_mid', v)} placeholder="中期症状" />
                        <Input label="末期" value={form.violation_late}
                            onChange={v => set('violation_late', v)} placeholder="末期症状" />
                    </div>
                </div>
                ...
                <div style={S.section}>
                    ...
                    <DynamicList label="目撃証言" items={form.testimonies} itemKey="testimonies"
                        placeholder="証言" />
                    <DynamicList label="記録媒体URL" items={form.media_urls} itemKey="media_urls"
                        placeholder="URL" hint="画像・動画・音声・文章のURL" />
                </div>
                ...
                <div style={S.section}>
                    ...
                    <TextArea label="その場の回避" value={form.avoidance}
                        onChange={v => set('avoidance', v)} placeholder="例：視線を切る / 数を数える / 音を止める" />
                    <TextArea label="二次被害防止" value={form.secondary_prevention}
                        onChange={v => set('secondary_prevention', v)} placeholder="例：拡散しない / URLを共有しない" />
                    <TextArea label="追加調査メモ" value={form.investigation_notes}
                        onChange={v => set('investigation_notes', v)} placeholder="次に検証すべきポイント" />
                </div>
                ...
                <div style={S.section}>
                    ...
                    <div style={S.row}>
                        <Input label="関連怪異" value={form.related_anomalies}
                            onChange={v => set('related_anomalies', v)} placeholder="TMP-???" />
                        <Input label="関連キャラ" value={form.related_characters}
                            onChange={v => set('related_characters', v)} placeholder="CHAR-???" />
                        <Input label="関連組織" value={form.related_factions}
                            onChange={v => set('related_factions', v)} placeholder="FAC-???" />
                        <Input label="関連用語" value={form.related_terms}
                            onChange={v => set('related_terms', v)} placeholder="用語リンク" />
                    </div>
                </div>
